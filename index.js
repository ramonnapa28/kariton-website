...
app.post('/dashboard/account/add-admin', async (req, res) => {
  ...
  app.get('/dashboard/barangay-status',(req,res)=>{
    res.render('barangay-status.ejs')
  });

  app.get('/dashboard/barangaydeclined',async (req,res)=>{
    try {
      const barangay = await Barangay.find({isApproved: false });

      console.log(barangay);
      
      res.render('barangay-decline.ejs', {junk:barangay});
    } catch (error) {
  ...
  app.get('/dashboard/junkshop-status',(req,res)=>{
    res.render('junkshop-status.ejs')
  });


  app.get('/dashboard/junkshopdecline',async (req,res)=>{
    try {
      const junkshop = await JunkShop.find({isApproved: false });

      console.log(junkshop);
      
      res.render('junkshop-decline.ejs', {junk:junkshop});
    } catch (error) {
...
app.post('/junkshop/scraps', async (req, res) => {
  try {
    ...
    if (type === 'disable') {
     ...
     await AdminScrap.findByIdAndUpdate(
        scrapId,
        { status: type}, // Update the status
    );
